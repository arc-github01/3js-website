...
export function Model({ mouse = { x: 0, y: 0 }, isCanvasHovered = false, isDragging = false, ...props }) {
  const group = useRef()
  ...
  useFrame(() => {
    if (group.current) {
      if (isDragging) {
        // When dragging, let OrbitControls handle the rotation
        // No additional rotation applied
        return;
      }
      
      if (isCanvasHovered) {
        // When hovering but not dragging, gently follow cursor
        ...
        // When cursor is out of canvas, rotate clockwise subtly
        group.current.rotation.y += 0.0007; // Subtle clockwise rotation
      }
    }
  })
